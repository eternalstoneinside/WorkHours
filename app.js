...
function openSubmenu(menuId) {
	...
	if (menuId === "main-menu") {
		...
		backupMenu.style.display = "none";
	} else if (menuId === "backup-menu") {
		mainMenu.style.display = "none";
		reportMenu.style.display = "none";
		backupMenu.style.display = "flex";
	}
}
...
function createBackupData() {
	const entriesCount = Object.keys(workLogs).length;
	return {
...
function downloadBackup() {
	...
	const link = document.createElement("a");
	link.href = url;
	link.download = `WorkHours_Backup_${dateStr}.json`;
	link.click();
	URL.revokeObjectURL(url);

	toggleMenu();
	alert(`✅ Бекап збережено!\n\n📊 Записів: ${backupData.entriesCount}`);
}
...
function shareBackup() {
	...
	if (navigator.share) {
		...
		const file = new File(
			[blob],
			`WorkHours_Backup_${new Date().toISOString().split("T")[0]}.json`,
			{
				type: "application/json",
			},
		);
		...
		navigator
			.share({
				title: "WorkHours Backup",
				text: `Бекап WorkHours від ${dateStr}\nЗаписів: ${backupData.entriesCount}`,
				files: [file],
			})
			.then(() => {
				toggleMenu();
			...
			.catch((err) => {
				// Якщо не підтримується шаринг файлів - пробуємо текст
		...
		navigator.clipboard
			.writeText(JSON.stringify(backupData, null, 2))
			.then(() => {
				toggleMenu();
				alert(
					"📋 Бекап скопійовано в буфер обміну!\n\nВставте його в файл або повідомлення.",
				);
			})
			.catch(() => {
				alert("❌ Ваш браузер не підтримує Share API");
	...
	input.onchange = (e) => {
		...
		reader.onload = (event) => {
			try {
				...
				if (!backupData.workLogs || typeof backupData.workLogs !== "object") {
					alert("❌ Некоректний формат бекапу");
					return;
				}

				const backupDate = backupData.timestamp
					? new Date(backupData.timestamp).toLocaleString("uk-UA")
					: "невідома";
				const backupEntries = Object.keys(backupData.workLogs).length;
				const currentEntries = Object.keys(workLogs).length;

				const confirmed = confirm(
					`⚠️ Замінити всі дані?\n\n` +
						`📅 Дата бекапу: ${backupDate}\n` +
						`📊 Записів у бекапі: ${backupEntries}\n` +
						`📊 Поточних записів: ${currentEntries}\n\n` +
						`Ця дія замінить ВСІ поточні дані!`,
				);
				...
				if (confirmed) {
					...
					renderCalendar();
					toggleMenu();
					alert(`✅ Дані відновлено з бекапу!\n\nЗаписів: ${backupEntries}`);
				}
			} catch (err) {
	...
	input.onchange = (e) => {
		...
		reader.onload = (event) => {
			try {
				...
				// Підрахунок нових та конфліктних записів
				let newEntries = 0;
				let conflictEntries = 0;
				for (let date in backupData.workLogs) {
					if (workLogs[date]) {
						conflictEntries++;
					} else {
						newEntries++;
				...
				const confirmed = confirm(
					`🔀 Об'єднати дані?\n\n` +
						...
						`📊 Записів у бекапі: ${backupEntries}\n` +
						`📊 Поточних записів: ${currentEntries}\n\n` +
						`➕ Нових записів: ${newEntries}\n` +
						`⚠️ Конфліктів (буде замінено): ${conflictEntries}\n\n` +
						`Існуючі записи будуть перезаписані даними з бекапу!`,
				);

				if (confirmed) {
					// Merge: бекап перезаписує існуючі дані
					workLogs = { ...workLogs, ...backupData.workLogs };
					localStorage.setItem("workLogs", JSON.stringify(workLogs));
					renderCalendar();
					toggleMenu();

					const totalNow = Object.keys(workLogs).length;
					alert(
						`✅ Дані об'єднано!\n\n` +
							`➕ Додано: ${newEntries}\n` +
							`🔄 Оновлено: ${conflictEntries}\n` +
							`📊 Всього записів: ${totalNow}`,
					);
				}
			} catch (err) {
				alert("❌ Помилка читання файлу: " + err.message);
